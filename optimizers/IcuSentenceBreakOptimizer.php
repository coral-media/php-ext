...
class IcuSentenceBreakOptimizer extends OptimizerAbstract
{
    /**
    ...
    public function optimize(array $expression, Call $call, CompilationContext $context)
    {
        if (!isset($expression['parameters']) || count($expression['parameters']) !== 2) {
            throw new CompilerException(
                "'icu_sentence_break' requires exactly 2 parameters (text, locale)",
                $expression
            );
        }
        ...
        // Generate C code: icu_sentence_break(Z_STR_P(text), Z_STRVAL_P(locale), &return_value)
        $context->codePrinter->output(
            sprintf(
                "icu_sentence_break(Z_STR_P(%s), Z_STRVAL_P(%s), &%s);",
                $params[0],
                $params[1],
                $symbol->getName()
            )
